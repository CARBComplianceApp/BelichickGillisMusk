...
- `npx tsc --noEmit` reports `TS1382` for a bare `>` in JSX text in `components/Dashboard.tsx`. This is **not** part of the build pipeline — Vite/esbuild compiles and runs the app fine. Do not "fix" that line; it is harmless.

### Non-obvious gotchas
- `index.html` must contain `<script type="module" src="/index.tsx"></script>` as the entry point. AI Studio injects this at runtime, but local Vite does not, so without it the page renders a **blank/black screen** with an empty `#root`. This entry script is committed on the setup branch.
